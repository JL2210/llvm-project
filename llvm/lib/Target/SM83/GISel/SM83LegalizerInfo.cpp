...
SM83LegalizerInfo::SM83LegalizerInfo(const DataLayout DL) : LegalizerInfo() {
  ...
  const LLT p1 = LLT::pointer(1, DL.getPointerSizeInBits(1));
  ...
  getActionDefinitionsBuilder({G_UADDE, G_UADDO})
      .legalFor({s1, s8});
  ...
  getActionDefinitionsBuilder(G_INTTOPTR)
      ...
      .legalForCartesianProduct({p1}, {s8})
      .customForCartesianProduct({p1}, {s16});

  getActionDefinitionsBuilder(G_PTRTOINT)
      .legalForCartesianProduct({s16}, {p0, p1})
      .legalForCartesianProduct({s8}, {p1});
